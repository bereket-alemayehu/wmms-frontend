...
import { mockRefunds, mockTickets } from "@/lib/mock-data"
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">View your refund status and details</p>
        </div>
        <div className="flex items-center gap-4 text-sm">
          <div className="text-right">
            <p className="text-2xl font-bold text-foreground">{customerRefunds.length}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="bg-card border-border">
          ...
          <CardContent>
            <p className="text-2xl font-bold text-card-foreground">{approvedRefunds.length}</p>
        ...
        <Card className="bg-card border-border">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium text-card-foreground">Total Amount</CardTitle>
              <DollarSign className="w-4 h-4 text-primary" />
            </div>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold text-card-foreground">{totalRefundAmount} ETB</p>
            <p className="text-xs text-muted-foreground mt-1">Approved refunds</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground mt-1">All your refunds processed automatically</p>
        </CardHeader>
        <CardContent>
          {customerRefunds.length === 0 ? (
            <div className="text-center py-12">
              <DollarSign className="w-16 h-16 mx-auto text-muted-foreground mb-4" />
              <p className="text-lg font-medium text-foreground mb-2">No refunds yet</p>
              <p className="text-sm text-muted-foreground">
                Refunds are automatically processed when eligible tickets meet the criteria.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {customerRefunds.map((refund) => {
                ...
                const updatedDate = refund.updatedAt !== refund.createdAt
                ...
                return (
                  <div key={refund._id} className="p-4 bg-secondary rounded-lg border border-border hover:border-primary/30 transition-colors">
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <Badge
                            ...
                            className={cn(
                              "font-medium text-sm",
                              statusColors[refund.status] || "bg-secondary text-secondary-foreground border-border"
                            )}
                          >
                            {refund.status}
                          </Badge>
                          <span className="text-xl font-bold text-secondary-foreground">{refund.amount} ETB</span>
                        </div>
                      ...
                      {ticket && (
                        <div className="space-y-1 pt-2 border-t border-border">
                          <p className="text-sm text-muted-foreground">
                            <span className="font-medium text-secondary-foreground">Ticket Category:</span> {ticket.category}
                          </p>
                        ...
                        <div className="mt-2 p-3 bg-card rounded border border-border">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">{refund.adminComment}</p>
                            </div>
                          </div>
                        </div>
                      )}
